<div   style="border: 2px solid #E1E6EA;padding: 10px 15px 10px 15px;max-width: 600px;justify-content: center"  >
    ...
    <p>
        ¡Hola <?=$pedido->cliente_nombre?>! su pedido a sido confirmado como entregado, Gracias por usar Data Center
        <br>
        <br>
        <div>
            <h5>Pedido # <?= h($pedido->id_pedido) ?></h5>
            <div>Estado: ENTREGADO</div>
            <div>Total: <?= $pedido->total ?> </div>
            <div>Metodo de Entrega: <?= $pedido->metodo_entrega ?> <?= $pedido->metodo_entrega == 'DELIVERY' ? '(Pago contra entrega)' : ''?> </div>
        </div>
